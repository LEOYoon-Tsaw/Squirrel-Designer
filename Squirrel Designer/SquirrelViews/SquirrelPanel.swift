...
final class SquirrelPanel: NSPanel {
    ...
    private var selRange: NSRange = .init()
    ...
    override func sendEvent(_ event: NSEvent) {
        ...
        case .leftMouseUp:
            ...
            if let index, index == self.index && index >= 0 && index < candidates.count {
                if index < view.viewModel.inputTemplate.candidates.count {
                    view.viewModel.inputTemplate.selection = index
                }
                print("Candidate selected at \(index)")
            }
        case .mouseEntered:
        ...
        case .scrollWheel:
            if event.phase == .began {
                ...
                if abs(scrollDirection.dx) > abs(scrollDirection.dy) && abs(scrollDirection.dx) > 10 {
                    print(((scrollDirection.dx < 0) == vertical) ? "Paging Up triggered" : "Paging Down triggered")
                } else if abs(scrollDirection.dx) < abs(scrollDirection.dy) && abs(scrollDirection.dy) > 10 {
                    print((scrollDirection.dy > 0) ? "Paging Up triggered" : "Paging Down triggered")
                }
                scrollDirection = .zero
                // Mouse scroll wheel
    ...
    func update(inputTemplate: InputTemplate) {
        var candidates = [String]()
        var labels = [String]()
        var comments = [String]()
        for cand in inputTemplate.candidates {
            candidates.append(cand.text)
            labels.append(cand.label)
            comments.append(cand.comment)
        }
        self.update(preedit: inlinePreedit ? "" : inputTemplate.preedit.preedit, selRange: inputTemplate.preedit.selRange,
                    caretPos: inputTemplate.preedit.selRange.upperBound, candidates: candidates, comments: comments, labels: labels,
                    highlighted: inputTemplate.selection, page: 1, lastPage: false, update: true)
    }
...
private extension SquirrelPanel {
    ...
    func show() {
        ...
        let position = NSRect(x: frame.minX, y: frame.maxY, width: 0, height: 0)
        ...
        if vertical {
            ...
            // Removed position adjusting dealing with vertical position and preedit area.
            panelRect.origin.y = position.minY - SquirrelTheme.offsetHeight - panelRect.height + theme.pagingOffset
            panelRect.origin.x = position.minX - SquirrelTheme.offsetHeight
