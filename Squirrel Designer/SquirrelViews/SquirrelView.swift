...
@MainActor private class SquirrelLayoutDelegate: NSObject, @MainActor NSTextLayoutManagerDelegate {
...
final class SquirrelView: NSView {
    ...
    var darkThemeAvailable: Bool {
        viewModel.squirrelSetting.style.darkScheme != nil
    }
    var currentTheme: SquirrelTheme {
        let colorScheme: String
        switch viewModel.selection {
        case .colorScheme(let index):
            colorScheme = viewModel.squirrelSetting.colorSchemes[index].codeName
        case .style, .template, .allSettings:
            if isDark {
                colorScheme = viewModel.squirrelSetting.style.darkScheme ?? viewModel.squirrelSetting.style.lightScheme
            } else {
                colorScheme = viewModel.squirrelSetting.style.lightScheme
            }
        }
        return SquirrelTheme(setting: viewModel.squirrelSetting, colorScheme: colorScheme)
    }
    ...
    func convert(range: NSRange) -> NSTextRange? {
        guard range != .init() else { return nil }
